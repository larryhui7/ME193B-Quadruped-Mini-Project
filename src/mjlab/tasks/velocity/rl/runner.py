...
class VelocityOnPolicyRunner(OnPolicyRunner):
  ...
  def save(self, path: str, infos=None):
    ...
    # Only export ONNX + log if WandB is actually active
    if self.logger is None:
      return

    # Check logger type robustly
    if self.logger.__class__.__name__.lower().startswith("wandb"):
        policy_path = path.split("model")[0]
        filename = os.path.basename(os.path.dirname(policy_path)) + ".onnx"

        # Optional observation normalizer
